...
echo "N_NODE = ${N_GPU} ; MODEL_SIZE = ${MODEL_SIZE} ; GLOBAL_BATCH = ${GLOBAL_BATCH} ; ZERO_STAGE = ${ZERO_STAGE}" >> result.log.txt
...
MICRO_BATCH=$((GLOBAL_BATCH/8/N_GPU))
...
${PREFIX} deepspeed --force_multi --num_nodes=$N_NODE --hostfile=hostfile pretrain_bert.py \
    --bert-no-binary-head \
    --cpu-optimizer \
