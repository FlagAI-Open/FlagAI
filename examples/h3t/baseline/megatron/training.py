...
def training_log(loss_dict, total_loss_dict, learning_rate, iteration,
    ...
    if iteration % args.log_interval == 0:
        _mem = torch.cuda.max_memory_allocated() / (2**30)
        _time_f = timers("forward-compute").elapsed(reset = False)
        _time_b = timers("backward-compute").elapsed(reset = False)
        _time_b += timers("backward-embedding-all-reduce").elapsed(reset = False)
        _time_o = timers("optimizer").elapsed(reset = False)
        _time = _time_f + _time_b + _time_o
        buf = "step = %d | step time : %.3f s | forward time : %.3f s | backward time : %.3f s | optimize time : %.3f s | memory : %.1f GiB" % \
                (iteration, _time / args.log_interval, _time_f / args.log_interval, _time_b / args.log_interval, _time_o / args.log_interval, _mem)
        print_rank_0(buf)
        if args.rank == 0:
