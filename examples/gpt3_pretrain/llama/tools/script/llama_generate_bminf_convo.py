...
state_dict = "/data/ldwang/checkpoints_pred"
...
for text in texts:
    ...
    from conversations_dataset import _add_speaker_and_signal
    header = "A chat between a curious user and an artificial intelligence assistant. " \
             "The assistant gives helpful, detailed, and polite answers to the user's questions.\n"
    obj = pack_obj(text)
    print(f"obj {obj}")
    _ = _add_speaker_and_signal(header, obj, get_conversation=True)
    print(f"convo_obj {obj}")
    delete_last_bot_end_singal(obj)
    print(f"convo_obj delete_last_bot_end_singal {obj}")
    tokens = convo_tokenize(obj, tokenizer)
