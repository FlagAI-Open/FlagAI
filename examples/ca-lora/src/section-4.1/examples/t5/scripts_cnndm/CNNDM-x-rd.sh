...
BASE_PATH="/local/apps/calora" # to change
VERSION="3b"
DATASET="CNNDM"
...
OPTS+=" --model-config /mnt/data/user/tc_agi/user/zhaoweilin/t5-${VERSION}" # to change
OPTS+=" --batch-size 4"
...
OPTS+=" --inherit-ckpt-path /local/apps/calora/inherit/CNNDM.pt" # to change
...
CMD="python3 -m torch.distributed.launch ${DISTRIBUTED_ARGS} ${BASE_PATH}/examples/t5/finetune_t5_cnndm.py ${OPTS}"
echo ${CMD}

${CMD} 2>&1  | tee /data/checkpoints/logs/t5_cnndm/${DATASET}-${SAVE_NAME}.log
