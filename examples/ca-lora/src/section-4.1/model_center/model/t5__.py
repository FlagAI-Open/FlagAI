...
class T5(BaseModel): 
    ...
    def forward(self, 
        ...
        if encoder_outputs is None:
            ...
            self.hidden_states_enc = hidden_states_enc
            self.enc_attention_mask = enc_attention_mask
            self.enc_position_bias = enc_position_bias
            encoder_outputs, encoder_mid = self.encoder(hidden_states_enc, enc_attention_mask, enc_position_bias)
        else:
        ...
        self.hidden_state_dec = hidden_states_dec
        self.dec_attention_mask = dec_attention_mask
        self.dec_position_bias = dec_position_bias
        ...
        if use_cache:
            ...
            decoder_outputs, decoder_mid = self.decoder(hidden_states_dec, dec_attention_mask, dec_position_bias,
                                           encoder_outputs, cross_attention_mask, None)
        ...
        if output_logits:
            ...
            if self.config.scale:
                logits = logits * self.config.dim_model ** -0.5

        if not return_dict:
            return decoder_outputs, encoder_outputs, current_key_values, logits, encoder_mid, decoder_mid, None, None, None
        else:
            return Seq2SeqModelOutput(
                ...
                logits = logits,
                encoder_hidden_states=encoder_mid,
                decoder_hidden_states=decoder_mid,
