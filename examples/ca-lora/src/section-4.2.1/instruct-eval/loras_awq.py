...
class LowRankLinear(nn.Module):
    ...
    def __init__(self,
        ...
        if r > 0:
            self.lora_A = nn.Parameter(torch.zeros((r, in_features), device=weight.device, dtype=torch.half))
            self.lora_B = nn.Parameter(torch.zeros((out_features, r), device=weight.device, dtype=torch.half))
...
class LowRankLinearActivate(nn.Module):
    ...
    def __init__(self,
        ...
        if lora_dropout > 0.:
            ...
            self.lora_dropout = lambda x: x
        if r > 0:
            self.lora_a1 = nn.Parameter(torch.zeros((r, in_features), device=weight.device, dtype=torch.half))
            self.lora_a2 = nn.Parameter(torch.zeros((out_features, r), device=weight.device, dtype=torch.half))
            self.scaling = self.lora_alpha / self.r
