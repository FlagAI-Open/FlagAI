...
LOGFILE=/data2/logs/$configfile.log.txt
echo "LOGFILE", $LOGFILE

for ((i=1;i<=$app_num;i++ )); do
    ip=`sed -n $i,1p $hostfile|cut -f 1 -d" "`
    echo "ip", $ip
    ssh $ip "cd $WORKSPACE; bash bmtrain_mgpu_flash_code.sh $hostfile $configfile $model_name $exp_name $exp_version 1>$LOGFILE 2>&1 &" &
