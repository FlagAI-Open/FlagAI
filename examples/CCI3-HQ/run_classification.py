...
class ModelArguments:
    ...
    freeze_backbone: int = field(
        default=0, metadata={"help": "Whether to freeze the backbone or not."}
    )
    model_name: str = field(
        default="xx", metadata={"help": "Whether to freeze the backbone or not."}
    )
...
def get_parameter_number(model):
    ...
    logger.info(
        f"Total param: {total_num}, trainable num is: {trainable_num}, trainable ratio is: {trainable_num/total_num*100.0}%"
    )
...
def main():
    ...
    # 回归任务不用dp，分类任务用dp
    if is_regression:
        if "bert" in model_args.model_name.lower() or "roberta" in model_args.model_name.lower():
    ...
    if model_args.freeze_backbone > 0:
        if "roberta" in model_args.model_name.lower():
            for param in model.roberta.embeddings.parameters():
                param.requires_grad = False
            for param in model.roberta.encoder.parameters():
                param.requires_grad = False
        elif "bert" in model_args.model_name.lower():
            for param in model.bert.embeddings.parameters():
                param.requires_grad = False
            for param in model.bert.encoder.parameters():
                param.requires_grad = False
        ## freeze之后qwen的可更新参数很少
    ...
    if training_args.do_train and not is_regression:  # classification, training
        ...
        logger.info(f"label_to_id: {model.config.label2id}")
    ...
    def compute_metrics(eval_pred):
        ...
        ## Binary: a threshold of 3
        logits, labels = eval_pred
        labels = np.round(labels.squeeze()).astype(int)
        labels = np.where(labels >= 3, 0, 1)
        preds = np.where(logits.squeeze() >= 3, 0, 1).clip(0, 5).astype(int)
        report = classification_report(labels, preds)
        print("Binary Validation Report:\n" + report, flush=True)
        matrix = confusion_matrix(labels, preds)
        print("Binary Confusion Matrix:\n" + str(matrix), flush=True)
        ...
        result['pos_precision'] = precision
        result['pos_recall'] = recall
        result['pos_f1'] = f1_score
        result['neg_precision'] = precision_negative
        result['neg_recall'] = recall_negative
        result['neg_f1'] = f1_score_negative
    ...
    if data_args.pad_to_max_length:
        ...
        data_collator = DataCollatorWithPadding(
            tokenizer, padding=True, max_length=max_seq_length, pad_to_multiple_of=8
        )
    else:
        data_collator = None
