...
print("Pleasure ensure you have clone the repo: https://github.com/taoyds/test-suite-sql-eval")

sys.path.insert(0, "./test-suite-sql-eval")
...
def eval_order(pred, label):
    ...
    if len(label['orderBy']) > 0:
        label_total = 1
    if len(
        label['orderBy']) > 0 and pred['orderBy'] == label['orderBy'] and (
        ...
        for idx, pg in enumerate(zip(p, g)):
            ...
            if etype in ["all", "exec"]:
                exec_score = eval_exec_match(
                    db=db, p_str=p_str, g_str=g_str, plug_value=plug_value, keep_distinct=keep_distinct,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gold', dest='gold', type=str, help="the path to the gold queries",
                        default="./evaluation_examples/gold.txt")
    parser.add_argument(
        '--pred', dest='pred', type=str, help="the path to the predicted queries",
        default="./evaluation_examples/predict.txt")
    parser.add_argument(
        '--db', dest='db', type=str, help="the directory that contains all the databases and test suites",
        default="/data/shixiaofeng/data/text2sql/database")
    parser.add_argument('--table', dest='table', type=str, help="the tables.json schema file", default="./tables.json")
    parser.add_argument(
        '--etype', dest='etype', type=str, default='all',
        help="evaluation type, exec for test suite accuracy, match for the original exact set match accuracy",
        choices=('all', 'exec', 'match'))
    parser.add_argument(
        '--plug_value', default=False, action='store_true',
