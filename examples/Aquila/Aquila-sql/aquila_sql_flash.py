...
if env_args.enable_flash_attn_models and env_args.bmt_pre_load:
    ...
    gc.collect()
    torch.cuda.empty_cache()
    print('*'*20, "flash att model after load", model)

elif env_args.bmt_pre_load:

    model.load_weights(checkpoint_path)
    print('*'*20, "flagai model load weights successfully...")
...
## 预训练部分代码
if env_args.pretrain_data:

    data_prefix = env_args.enable_pretrain_dataset_prefix
    data_impl = 'mmap'
    splits_string = '90,10'
    # train_valid_test_num_samples = [90, 10]
    train_valid_test_num_samples = [100,0]
    seq_length = 1024
    seed = 2023
    skip_warmup = True
...
## conversations_dataset
    ...
    print(f"Unvalid: enable_sft_conversations_dataset_v2 is [{env_args.enable_sft_conversations_dataset_v2}]")
    sys.exit()
